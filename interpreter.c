...
int mount(char *name, int blocksize, int numblocks)
{
  int errCode = partition(name,blocksize,numblocks);
  if (errCode == 0)
  {
    errCode = mountFS(name);
  }
  return errCode;
}
...
{
  int code = openfile(filename);
  int errCode = writeBlock(code, words);
  return errCode;
}
...
int interpreter(char* words[]){
    ...
    if ( strcmp(words[0],"help") == 0 ) {
        ...
        // check if there's at least 3 arguments and not >= 4 arguments
        if ( strcmp(words[2],"_NONE_") == 0  || strcmp(words[4],"_NONE_") != 0 ) return -2;


        errorCode = mount(words[1], atoi(words[3]), atoi(words[2]));
        ...
        int i = 2;
        char *dest;
        while(strcmp(words[i],"NONE")!=0)
        {
          strncat(dest, words[i], 999);
        }
        


        errorCode = write(words[1],dest);

    }else if ( strcmp(words[0],"read") == 0 ) {
        // if it's the "read" command
        // check if there's 2 inputs exactly
        if ( strcmp(words[1],"_NONE_") == 0  || strcmp(words[2],"_NONE_") == 0 || strcmp(words[3],"_NONE_") != 0) return -2;


        errorCode = read(words[1],words[2]);
